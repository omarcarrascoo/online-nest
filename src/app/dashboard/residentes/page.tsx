...
function unitBadge(unit?: string) {
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] border border-white/15 bg-white/10 text-white/80">
      {unit || "—"}
    </span>
  );
}
...
export default function ResidentsExplorer() {
  ...
  // filtros rápidos
  const [onlyWithDebt, setOnlyWithDebt] = useState(false); // heuristic: 0 pagos o último pago viejo
  ...
  const filtered = useMemo(() => {
    ...
    let base = residents.filter((r) => {
      ...
      if (onlyWithDebt) {
        ...
        // heurística: si no hay pagos o el último pago es hace más de 45 días => deudoso
        const overdue =
      ...
      <section className={`relative ${glass} px-4 py-4`}>
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-xs text-white/60">Residentes</p>
            <p className="text-lg text-white/90">{metrics.total}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-xs text-white/60">Pagos totales</p>
            <p className="text-lg text-white/90">{metrics.totalPayments}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-xs text-white/60">Monto recaudado</p>
            <p className="text-lg text-white/90">{money(metrics.totalMXN)}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            <p className="text-xs text-white/60">Sin pagos</p>
            <p className="text-lg text-white/90">{metrics.withNoPayments}</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className={`${ghostBtn} ${onlyWithDebt ? "ring-2 ring-amber-300/30" : ""}`}
      ...
      {view === "cards" && (
        <section className={`relative ${glass} p-4`}>
          {loading ? (
            ...
            <div className="p-6 text-white/70">No se encontraron residentes.</div>
          ) : (
            <ul className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {filtered.map((r) => {
                ...
                return (
                  <li
                    key={r.id}
                    className="relative rounded-xl border border-white/10 bg-white/5 p-4 hover:bg-white/10 transition"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-white/90 font-medium truncate">{r.fullName}</p>
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] border border-white/15 bg-white/10 text-white/80">
                          {r.payments.length} pagos
                        </span>
                        {risk ? (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] border border-amber-400/30 bg-amber-600/20 text-amber-100">
                            en riesgo
                          </span>
                        ) : null}
                      </div>
                    ...
                    <div className="mt-3 flex items-end justify-between gap-3">
                      <div className="text-xs text-white/70">
                        <p>Último pago: <span className="text-white/90">{toYmd(last)}</span></p>
                        <p>Total: <span className="text-white/90">{money(sumPayments(r.payments))}</span></p>
                      </div>
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {Object.entries(
                ...
                .map(([letter, group]) => (
                  <div key={letter} className="relative rounded-xl border border-white/10 bg-white/5 p-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="h-6 w-6 rounded-md bg-white/10 border border-white/10 text-white/80 text-sm flex items-center justify-center">
                        {letter}
                      </div>
                      <span className="text-xs text-white/60">{group.length} resultado(s)</span>
                    </div>
                    <ul className="space-y-2 max-h-80 overflow-y-auto pr-1">
                      {group.map((r) => (
                        <li key={r.id} className="rounded-lg border border-white/10 bg-white/5 p-2">
                          <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0">
                              <p className="text-white/90 text-sm truncate">{r.fullName}</p>
                              <p className="text-white/60 text-[11px] truncate">{r.email || "—"}</p>
                            </div>
                            <div className="ml-auto">{unitBadge(r.unitNumber)}</div>
                          </div>
                          <div className="mt-1 flex items-center justify-between">
                            <span className="text-[11px] text-white/70">{r.payments.length} pagos</span>
                            <Link
                              href={`/dashboard/residentes/${r.id}`}
                              className="inline-flex items-center gap-1 rounded-md border border-white/15 bg-white/5 px-2 py-1 text-[11px] text-white/90 hover:bg-white/10"
                            >
                ...
                .map((r) => {
                  ...
                  return (
                    <li key={r.id} className="relative ml-2 pl-4 py-3">
                      {/* punto */}
                      <span className={`absolute left-[-7px] top-4 h-3 w-3 rounded-full ${risk ? "bg-amber-300" : "bg-teal-300"}`} />
                      <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-white/60 text-xs">
                              Último pago: <span className="text-white/90">{toYmd(last)}</span>
                            </p>
                          </div>
                          <div className="ml-auto flex items-center gap-2">
                            {unitBadge(r.unitNumber)}
                            <span className="text-xs text-white/70">{r.payments.length} pagos</span>
                            <span className="text-xs text-white/90">{money(total)}</span>
                            <Link
                              href={`/dashboard/residentes/${r.id}`}
                              className="inline-flex items-center gap-1 rounded-lg border border-white/15 bg-white/5 px-3 py-1.5 text-xs text-white/90 hover:bg-white/10"
                            >
